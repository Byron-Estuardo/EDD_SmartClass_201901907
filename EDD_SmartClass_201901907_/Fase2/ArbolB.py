...
class arbolB:
            ...
            if self.comparador == False:
                if self.estado:
                    ...
                    if self.sube:
                        if raiz.cuenta < 4:
                            self.sube = False
                            self.MeterHoja(raiz, posicion, self.Codigo, self.Nombre, self.Creditos, self.Codigos, self.Obligatorio)
                        else:
                            self.sube = True
                            self.DividirPaginaB(raiz, posicion, self.Codigo, self.Nombre, self.Creditos, self.Codigos, self.Obligatorio)
            else:
                print("Dato Repetido")
                self.comparador = False
    ...
    def graficar(self):
        ...
        self.grafica += "\n}\n"
        ruta = "C:\\Users\\Diego Camey\\Desktop\\Reportes_F2"
        os.makedirs(ruta, exist_ok=True)
        nombre = "ArbolCursosPensum" + str(contadorGrafos)
        Archivo = open(ruta+"\\"+nombre+".dot", "w")
        Archivo.write(self.grafica)
        Archivo.close()
        instrucciones = "dot -Tpdf "+nombre+".dot -o "+nombre+".pdf"
        os.system("cd C:/Users/Diego Camey/Desktop/Reportes_F2/ " +
                  "&& " + instrucciones)
        contadorGrafos += 1
        ...
        if pagina is not None:
            if pagina.getCodigo(0) is not None:
                if pagina.getCodigo(0) != "":
                    if pagina.getApuntador(0) is not None and pagina.getApuntador(0).getCodigo(0) is not None and pagina.getApuntador(0).getCodigo(0) != "":
